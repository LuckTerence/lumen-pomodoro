...
1. **新用户热力图大面积空白 [中]** — `GetHeatmapData` 固定生成 365 天数据，应用使用不满一年时热力图绝大部分为空灰色格子，视觉上像显示异常。修复：根据最早一条已完成 session 的日期动态计算范围，最短 3 个月（90 天）、最长 12 个月（365 天），使热力图列数与实际使用时长匹配。

### 验证结果

- `dotnet build`：通过，0 warning / 0 error。
- `dotnet test`：通过，34/34。
...
1. **标题栏按钮仍不显示 [高]** — `NavigationView.TitleBar` 在当前底部导航布局下未实际渲染，导致最小化、最大化、关闭按钮仍不可见。修复：改为显式自定义顶栏，右侧放置最小化、最大化、关闭按钮，左侧标题区域负责拖动和双击最大化/还原。

### 验证结果

- `dotnet build LumenPomodoro.sln`：通过，0 warning / 0 error。

## [2026-05-09] Bug 修复 — 标题栏拖动行为
...
1. **窗口无法拖动 [高]** — WPF-UI 自定义 `TitleBar` 显示了窗口按钮，但未把标题栏空白区域接到窗口拖动逻辑。修复：标题栏左键按下时调用 `DragMove()`，双击标题栏最大化/还原，并跳过最小化、最大化、关闭按钮区域，避免抢按钮点击。
...
1. **热力图日期错位 [高]** — 原实现按数据下标 `i / 7`、`i % 7` 直接排格子，没有按真实星期补齐起始空位，导致月份标签、星期行和日期格子错位。修复：按真实日期计算自然周列和周一到周日行，月份标签按该月第一次出现的日期定位。
2. **热力图渲染稳定性 [中]** — 原实现强制把主题强调色转换为 `SolidColorBrush`，主题资源不是纯色刷时可能渲染异常。修复：增加安全取色兜底，并为画布开启裁剪，避免窄窗口下溢出。
